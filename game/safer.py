...
from .enemy import EnemyBase, find_closest_enemy


class Safer(Entity):
    def __init__(self, player: "Player") -> None:
        model = ASSETS.resolve_model("safer")
        super().__init__(
            model=model or "cube",
            color=color.rgb(240, 220, 120) if not model else color.white,
            scale=(0.8, 0.4, 0.8),
        )
        self.player = player
    ...
    def update(self, dt: float, enemies: list[EnemyBase]) -> None:
        ...
        if self.support_timer <= 0:
            ...
            self.player.restore_shield(settings.SAFER_REPAIR_AMOUNT)
            self.support_timer = settings.SAFER_SUPPORT_INTERVAL
