...
| --- | --- |
| Python | 3.10 이상 |
| Ursina | 5.0 이상 (`pip install ursina`) |
| panda3d-gltf | 0.13 이상 (`pip install panda3d-gltf`) |
| GPU | 통합 그래픽 이상 권장 |
...
> **Tip:** 가상환경을 사용하면 의존성 관리가 편리합니다. 창 제목은 `잔해의 노래 3D 프로토타입`이며 기본 해상도는 1600x900입니다. Blender 자산을 투입하지 않으면 자동으로 엔진 기본 메시가 사용됩니다.
...
| 마우스 좌클릭 | 근접 공격 |
| 마우스 우클릭 | 원거리 사격 |
| 마우스 휠 위/아래 | 추적 카메라 줌 인/아웃 |
| G | 근처 자원 채집 |
| C | 제작 패널 토글 |
| 1~4 | 제작 패널 활성화 시 해당 레시피 제작 |
| V | 전술 대시 |
| F1 / F2 / F3 | 스킬 트리 슬롯 해금 |
| R | 세션 재시작 |
| Esc | 게임 종료 |

### 4) 현재 구현된 시스템 요약

* **3D 탐사 공간:** 폐허 콜로니 외곽을 확장하고 Blender 자산이 존재하면 자동으로 교체 렌더링합니다. 랜덤 구조물이 엄폐 역할을 하며 추적 카메라가 부드럽게 따라옵니다.
* **환경 시뮬레이션:** 주야 사이클과 기후 컨트롤러가 장면 조명·안개 색·밀도를 조절합니다. 전투 압박과 분위기를 자연스럽게 변화시킵니다.
* **생존 지표 & 방어막:** 산소·에너지·체온·영양이 감소하며, 방어막은 일정 시간 후 재충전됩니다. 세이퍼는 에너지와 방어막을 주기적으로 보강합니다.
* **임무 & 성장:** 임무 로그가 스크랩·격추 수를 추적하고 보상을 제공합니다. 경험치는 스킬 트리를 해금해 근접 강화·대시 쿨다운·아이템 회복 버프를 제공합니다.
* **전투:** 근접/원거리 하이브리드 전투와 전술 대시를 지원합니다. 세이퍼는 자동 추적 사격을 수행하고 가디언급 `WardenSentinel` 보스가 확률적으로 등장합니다.
* **자원 & 제작:** 동적 자원 노드에서 스크랩·생존 자원을 채집하고 제작 패널에서 방어막 키트까지 조합할 수 있습니다. 스크랩 소비량과 보유량이 임무 진행과 연동됩니다.
* **재시작:** `R` 키로 세션을 초기화하면 월드·자원·날씨·미션 상태가 재구성됩니다.

### 5) Blender 자산 파이프라인

1. Blender에서 메시/애니메이션을 구성한 후 **File → Export → glTF 2.0 (.glb)**을 선택합니다.
2. `Format`은 `glTF Binary (.glb)`로, `Apply Modifiers`와 `+Y Up`을 체크하면 Ursina 좌표계와 일치합니다.
3. 저장 경로를 `assets/models/`로 지정하고 아래 키 이름과 매칭되는 파일명을 사용합니다.

| 키 | 기본 파일명 | 설명 |
| --- | --- | --- |
| player | `player.glb` | 플레이어 바디 및 애니메이션 |
| safer | `safer.glb` | 세이퍼 드론 |
| purge_drone | `purge_drone.glb` | 일반 적 드론 |
| warden | `warden.glb` | 정예/보스 유닛 |
| resource_scrap | `scrap_crate.glb` | 스크랩 상자 |
| resource_canister | `canister.glb` | 산소·에너지 캔 |
| structure | `structure.glb` | 폐허 구조물/바닥 |
| landing_pad | `landing_pad.glb` | 스타팅 플랫폼 |

4. 내보낸 파일이 존재하면 런타임에 자동으로 로드되며, 없으면 프로토타입 기본 메시를 사용합니다.
5. 고급 재질이나 애니메이션을 사용하려면 Blender에서 `Animation` 트랙을 포함한 상태로 내보내고, Ursina의 `Animator`를 활용해 후속 확장할 수 있습니다.

프로토타입은 "더미 스테이션" 이후 폐허 콜로니 외곽에서의 생존 시나리오를 3D로 미리 체험할 수 있도록 설계되었습니다. 향후 멀티플레이, 고급 AI, 스토리 미션을 확장하기 위한 기초 코드로 활용할 수 있습니다.
