...
> **새 소식** – 기획안 기반으로 구축한 실시간 3D 생존 액션 프로토타입(`main.py`)을 포함합니다. 파이썬과 [Ursina](https://www.ursinaengine.org/)만 설치하면 로컬에서 즉시 실행해 핵심 루프(3D 탐험, 생존 관리, 전투, 세이퍼 보조, 제작)를 체험할 수 있습니다. 자세한 실행 방법과 조작법은 [프로토타입 가이드](#프로토타입-가이드)를 참고하세요.
...
> **Tip:** 가상환경을 사용하면 의존성 관리가 편리합니다. 창 제목은 `잔해의 노래 3D 프로토타입`으로 표시되며, 기본 해상도는 1280x720입니다.
...
* **3D 탐사 공간:** 폐허 콜로니 외곽을 단일 구역으로 구축했습니다. 랜덤 구조물이 장애물 및 엄폐 역할을 하며, 세이퍼는 플레이어 주변을 선회합니다.
* **생존 지표:** 산소·에너지·체온·영양이 시간에 따라 감소하며, 0이 되면 초당 피해를 입습니다. 세이퍼는 주기적으로 에너지를 충전해 줍니다.
* **자원 노드:** 일정 시간마다 맵에 생성되며 `G`로 채집하면 스크랩 또는 생존 자원을 획득합니다. 스크랩은 제작 재료로 저장됩니다.
* **전투:** 정화 드론이 주기적으로 난입합니다. 마우스 좌클릭 근접 공격, 세이퍼 자동 사격, 적의 근접 공격 패턴을 포함합니다.
* **제작:** 스크랩을 소비해 산소통/배터리/히터/영양팩을 제작해 생존 지표를 회복할 수 있습니다. 제작 패널은 `C`로 열고 번호 키로 선택합니다.
* **재시작:** `R` 키로 언제든 세션을 초기화해 새로운 자원 배치를 받을 수 있습니다.
