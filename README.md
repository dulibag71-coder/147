...
> **새 소식** – 아래 기획을 토대로 한 2D 탑다운 생존 액션 프로토타입(`main.py`)을 함께 제공합니다. 파이썬과 [Pygame](https://www.pygame.org/news)만 설치되어 있다면 로컬에서 즉시 실행해 핵심 루프(자원 관리, 전투, 세이퍼 보조, 간이 제작)를 체험할 수 있습니다. 자세한 실행 방법과 조작법은 [프로토타입 가이드](#프로토타입-가이드)를 참고하세요.
...
| --- | --- |
| Python | 3.10 이상 |
| Pygame | 2.5 이상 (`pip install pygame`) |
...
> **Tip:** 가상환경에서 실행하면 의존성 관리가 편합니다. 창 제목은 `잔해의 노래: 더미 스테이션`으로 표시됩니다.
...
* **생존 지표:** 산소·에너지·체온·영양이 시간에 따라 감소하며, 0이 되면 초당 피해를 입습니다.
* **자원 노드:** 일정 시간마다 맵에 생성되며 `E`로 채집하면 스크랩 또는 생존 자원을 획득합니다.
* **전투:** 정화 드론이 주기적으로 난입합니다. `Space`로 근거리 공격, AI 동료 세이퍼는 자동 사격/지원으로 돕습니다.
* **세이퍼 보조:** 일정 주기로 에너지 회복 버프를 주고, 사정거리 내 적에게 자동으로 사격합니다.
* **제작:** 스크랩을 소모해 산소 탱크, 에너지 셀 등 생존 보급품을 제작할 수 있습니다.

프로토타입은 "더미 스테이션" 튜토리얼 구역을 가볍게 체험할 수 있도록 설계된 데모입니다. 향후 스테이지 확장, 세력 상호작용, 본격적인 멀티플레이 구현을 위한 기초 코드로 사용할 수 있습니다.
